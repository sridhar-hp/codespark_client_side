...
function ToggleSwitch({ checked, onChange, label }) {
  return (
    <button
      ...
      aria-label={label}
      onClick={onChange}
      className={`relative h-6 w-11 shrink-0 rounded-full transition-colors duration-200 outline-none focus-visible:ring-2 focus-visible:ring-[#F59E0B] focus-visible:ring-offset-2 focus-visible:ring-offset-[#0B1120] ${
        checked ? "bg-[#F59E0B]" : "bg-[#1F2937]"
      }`}
    >
      <motion.span
...
function NotificationCard({ item, onDismiss, onBookmark, onToggleRead, index }) {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -2 }}
      className={`group relative rounded-2xl border bg-[#111827]/70 p-4 backdrop-blur-sm transition-colors duration-300 sm:p-5 ${
        item.unread ? "border-[#1F2937]" : "border-[#1F2937]/60"
            ...
            {FILTER_CHIPS.map((chip) => {
              ...
              return (
                <button
                  ...
                  className={`relative rounded-full border px-3.5 py-1.5 text-xs font-medium outline-none transition-colors focus-visible:ring-2 focus-visible:ring-[#F59E0B] ${
                    active
