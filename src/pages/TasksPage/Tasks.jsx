        ...
        <div className="min-h-screen bg-[#0B1120] text-[#F9FAFB] font-sans antialiased relative">
            ...
            <div className="fixed bottom-6 right-6 z-50 flex flex-col gap-2 max-w-sm pointer-events-none">
                {toasts.map(toast => (
                    <div
                        key={toast.id}
                        className={`pointer-events-auto flex items-center gap-3 px-4 py-3 rounded-xl border shadow-2xl animate-slide-up duration-300 ${toast.type === 'xp'
                    ...
                    <div className="flex flex-wrap items-center gap-1.5 overflow-x-auto">
                        {['all', 'today', 'upcoming', 'completed', 'overdue'].map(tab => (
                            <button
                                ...
                                className={`px-4 py-2 text-xs font-bold uppercase tracking-wider rounded-xl transition-all cursor-pointer whitespace-nowrap ${selectedFilter === tab
                    ...
                    <div className="flex flex-wrap items-center gap-2 overflow-x-auto pb-1 [scrollbar-width:none]">
                        {INITIAL_CATEGORIES.map(cat => (
                            <button
                                ...
                                className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-full border text-xs font-semibold transition-all cursor-pointer whitespace-nowrap ${selectedCategory === cat.name
                            ...
                            <div className="space-y-3">
                                {filteredTasks.map((task, idx) => {
                                    ...
                                    return (
                                        <div
                                            key={task.id}
                                            className={`relative bg-[#111827]/80 backdrop-blur-md border p-5 rounded-2xl flex flex-col sm:flex-row sm:items-start justify-between gap-4 transition-all duration-300 group hover:scale-[1.005] ${isCompleted
                                        ...
                                        >
                                            <div className="flex items-start gap-4">
                                                ...
                                                <button
                                                    ...
                                                    className={`mt-1 h-5 w-5 rounded-md border flex items-center justify-center flex-shrink-0 transition-all cursor-pointer ${isCompleted
                                                ...
                                                <div className="space-y-1.5 max-w-lg">
                                                    <h3 className={`text-base font-bold transition-colors ${isCompleted ? 'text-[#6B7280] line-through' : 'text-white group-hover:text-amber-400'
                                                        }`}>
                                                        {task.title}
                                                    </h3>
                                                    ...
                                                    <div className="flex flex-wrap items-center gap-2 pt-1">
                                                        ...
                                                        <span className={`text-[10px] font-extrabold uppercase tracking-wider px-2 py-0.5 rounded-md ${task.priority === 'High'
                                            ...
                                            <div className="flex flex-col sm:items-end justify-between gap-3 text-right">
                                                ...
                                                <div className="flex items-center sm:justify-end gap-2">
                                                    ...
                                                    <span className={`text-xs font-extrabold px-2 py-0.5 rounded-lg ${isCompleted ? 'bg-slate-800/40 text-[#4B5563]' : 'bg-amber-500/10 text-amber-400 border border-amber-500/15 shadow-inner'
                                                        }`}>
                                                        +{task.xpReward} XP
                                                    </span>
                                                </div>
                                                ...
                                                <span className={`text-[10px] font-extrabold uppercase tracking-wider px-2.5 py-1 rounded-lg w-fit sm:ml-auto border ${isCompleted
                                ...
                                {routine.map(item => (
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            <div className={`h-4.5 w-4.5 rounded-md border flex items-center justify-center flex-shrink-0 transition-colors ${item.done
                                                ? 'bg-amber-500 border-amber-500 text-[#0B1120]'
                                                : 'bg-slate-900 border-[#1F2937] group-hover:border-amber-500'
                                                }`}>
                                                {item.done && <Check className="w-3 h-3 stroke-[3.5px]" />}
                                            </div>
                                            <span className={`text-xs font-semibold transition-all ${item.done ? 'text-[#6B7280] line-through' : 'text-white group-hover:text-white'
                                                }`}>
                                                {item.text}
                                            </span>
                                        </div>

                                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-lg ${item.done ? 'bg-[#1F2937] text-[#4B5563]' : 'bg-amber-500/10 text-amber-400'
                                            }`}>
                                            +{item.xp} XP
                                        </span>
                                    </button>
